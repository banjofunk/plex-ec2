...
module.exports.handler = async (event, context) => {
  ...
  const plexUserAuthToken = await fetch(`https://plex.tv/users/sign_in.json?user[login]=${username}&user[password]=${password}&X-Plex-Client-Identifier=plex-ec2`, {
    method: 'post'
  })
  .then(resp => resp.json())
  ...
  const ssmResp = await ssm.sendCommand(params).promise().catch(console.error);
